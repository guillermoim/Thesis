...
However, as shown by \citet{Alegre2022}, %one can utilize recent advancement in multi-objective RL
...
%While these and other conventional RL methods use Markovian reward functions, expressing a task with such reward function can be difficult and may not even be possible in some cases~\cite{Whitehead1995}. In settings where the reward function cannot be expressed in Markovian terms, task specification has raised especial interest in the last few years~\citep{Icarte2018, Camacho2019}.
...
The goal of the agent is to find an optimal policy $\pi^*$ that maximizes the expected discounted return, for any state-action pair ${(s,a)\in\cS\times\cA}$,
\begin{equation}
...
\begin{equation*}
  % \pi^*\in \argmax_\pi Q^\pi(s, a)\;\;\forall(s,a)\in\cS\times\cA 
...
A product MDP $\cM'$ is a well-defined MDP. The agent now follows a policy $\mu:\cU\times\cS\rightarrow\Delta(\cA)$, that depends both on the FSA state and the underlying MDP state. $\cM'$ can be solved with conventional RL methods such as Q-learning~\cite{Watkins1992} by finding an optimal policy $\mu^*$ that maximizes
\begin{equation*}
...
\subsubsection{Proof of optimality} We first restate the following theorem from~\citet{Alegre2022}.
