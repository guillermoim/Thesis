...
\begin{example}[1] %Returning to the example in Figure~\ref{fig:ex},
...
the hierachical approach needs five runs of the power method on significantly reduced %$(5+4)\times (5+4)$
 matrices (these runs can be parallelized), and one additional run on a $8\times 9$ (where the value of $G$ is known by definition) matrix, corresponding to~\eqref{eq:exits}.\\ %, to obtain the same global optimal solution.
